...
class MainWindow(QMainWindow):
    ...
    def show_teacher_info(self):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT tid from teachers")
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    SELECT c.cid, c.name, 
                        ...
                        t.tid, t.name as teacher_name, t.sex as teacher_sex
                    FROM clubs c
                    LEFT JOIN sc ON c.cid = sc.cid
                    LEFT JOIN tc ON c.cid = tc.cid
                    LEFT JOIN teachers t ON tc.tid = t.tid
                    GROUP BY c.cid, c.name, t.tid, t.name, t.sex
                """)
    ...
    def save_club_data(self, dialog):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                sql = """
                INSERT INTO clubs (cid, name)
                VALUES (%s, %s)
                """
                cursor.execute(sql, (club_id, name))
            connection.commit()
