...
void findCriticalPath(AdjList *graph) {
    ...
    // Identify and print the critical path
    printf("关键路径: ");
    for (int u = 0; u < graph->vexnum; u++) {
        ArcNode *p = graph->vertex[u].firstarc;
        while (p != NULL) {
            int v = p->adjvex;
            if (earliest[u] == latest[v] - p->weight) {
                printf("%c->%c ", graph->vertex[u].data, graph->vertex[v].data);
            }
            p = p->nextarc;
        }
    }
